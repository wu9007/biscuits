...
public class ServiceLogProxy {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (servletRequestAttributes != null) {
            ...
            logger.info("[{}: {}, {}: {}, {}: {}, {}: {}, {}: {}]", TRACE_ID, traceId, REQUEST_ID, requestId, TRANSACTION_ID, transactionId,
                    USER_NAME, userName, BUSINESS_DEPARTMENT_NAME, businessDepartmentName);
            if (traceId != null && userName != null) {
                ...
                this.executorService.execute(() -> {
                    ServiceLogView serviceLogView = new ServiceLogView().setTraceId(traceId).setRequestId(requestId).setServiceId(serviceId).setGlobalTransactionId(transactionId);
                    try {
                        serviceLogView.setUserName(URLDecoder.decode(userName, "UTF-8"));
                        if (businessDepartmentName != null) {
                            serviceLogView.setBusinessDeptName(URLDecoder.decode(businessDepartmentName, "UTF-8"));
                        }
